...
export default function AdminDashboard() {
  ...
  const handleExportData = () => {
    setIsExporting(true);
    setTimeout(() => {
      const headers = "ID,Name,Email,Class,Role,JoinedDate\n";
      ...
      const rows = users.map(u => `${sanitizeCsv(u.id)},${sanitizeCsv(u.name || 'N/A')},${sanitizeCsv(u.email)},${sanitizeCsv(u.education_level || '')},${sanitizeCsv(u.role)},${sanitizeCsv(new Date(u.created_at).toLocaleDateString())}`).join("\n");
