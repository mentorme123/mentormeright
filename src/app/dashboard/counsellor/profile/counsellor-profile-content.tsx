...
import { Loader2, Mail, Phone, User, MapPin, LogOut, X, Briefcase, Star } from "lucide-react";
...
export default function CounsellorProfileContent() {
  const router = useRouter();
  const supabase = createClient();
  const [loading, setLoading] = useState(true);
  const [profile, setProfile] = useState<UserProfile | null>(null);
  const [counselorInfo, setCounselorInfo] = useState<CounselorProfile | null>(null);
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-slate-500 text-sm mt-1">Update your counselor information.</p>
                </div>
      ...
      <div className="max-w-3xl mx-auto space-y-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-500">Manage your counselor account settings.</p>
          </div>
          <button
        ...
        <div className="bg-white rounded-2xl p-6 sm:p-8 border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-black text-slate-800 flex items-center gap-2">
              <User size={20} className="text-brand-blue" /> Counselor Information
            </h3>
            <button
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {[
              ...
              { icon: <User size={14} />, label: "Role", value: "Counselor" },
              { icon: <Briefcase size={14} />, label: "Specialization", value: counselorInfo?.specialization },
              { icon: <MapPin size={14} />, label: "Location", value: profile?.state && profile?.country ? `${profile.state}, ${profile.country}` : null },
              { icon: <Star size={14} />, label: "Rating", value: counselorInfo?.rating ? `${counselorInfo.rating}/5` : null },
