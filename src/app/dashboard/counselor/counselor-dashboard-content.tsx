...
export default function CounselorDashboardContent({ user }: CounselorDashboardProps) {
  const supabase = createClient();
  const [loading, setLoading] = useState(true);
  const [profileMenuOpen, setProfileMenuOpen] = useState(false);
  const [counselorName, setCounselorName] = useState(user.user_metadata?.full_name || user.email?.split('@')[0] || "Counselor");
              ...
              <div className="relative profile-dropdown">
                ...
                {profileMenuOpen && (
                  <div className="absolute right-0 mt-2 w-64 bg-white border border-slate-200 rounded-2xl shadow-2xl z-50 overflow-hidden">
                    ...
                    <div className="p-2">
                       <Link href="/dashboard/counsellor/profile" className="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-50 transition-colors text-left">
