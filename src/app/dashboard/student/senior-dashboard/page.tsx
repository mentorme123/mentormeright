...
export default function SeniorDashboard({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        console.error('SeniorDashboard: fetch error', error);
  ...
  const careerMatches = computedCareerMatches;

  const subjectReadiness = computedSubjectReadiness;
           ...
           <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
             <div>
               <h1 className="text-2xl sm:text-3xl font-black mb-1">
                 Student Career Dashboard – Class 11 & 12
               </h1>
