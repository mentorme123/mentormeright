  ...
  useEffect(() => {
    async function loadData() {
      ...
      setAuthUser({
        id: user.id,
        email: user.email || '',
        user_metadata: user.user_metadata as { full_name?: string; provider?: string; iss?: string }
      });
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        {assessmentStatus === 'completed' ? (
          ...
          <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
            ...
            <a href={`/career-assessment.html?email=${encodeURIComponent(authUser?.email || '')}&name=${encodeURIComponent(profile?.name || authUser?.user_metadata?.full_name || '')}&class=${encodeURIComponent(profile?.education_level || '')}&school=${encodeURIComponent(profile?.institution_name || '')}`} className="inline-block mt-4">
        ...
        {assessmentStatus === 'completed' && !profile?.has_paid_report && (
          <motion.div
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
              <div className="flex items-start gap-5">
                ...
                <div className="space-y-2">
                  <h3 className="text-2xl font-black">Get Your Detailed Career Report</h3>
                  <p className="text-blue-200 text-sm max-w-xl">
                    Unlock your comprehensive AI-generated career report with personalized recommendations, ideal career matches, higher education pathways, and skill development plans.
                  </p>
                  <div className="flex items-center gap-4 text-sm">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="w-2 h-2 bg-brand-orange rounded-full"></div>
                      <span className="font-bold">Skill Development Plan</span>
                    </div>
                  </div>
                </div>
              </div>
              <div className="flex flex-col items-end gap-3 shrink-0">
                 <div className="text-right">
                   <p className="text-[10px] font-bold text-blue-300 uppercase tracking-wider">One-time payment</p>
        ...
        {paymentSuccess && (
          ...
          >
            ...
            <p className="text-sm text-emerald-600 mt-1">Your detailed career report is now unlocked.</p>
          </motion.div>
        )}
        ...
        {assessmentStatus !== 'completed' ? (
          <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-sm text-center">
            ...
            <a href={`/career-assessment.html?email=${encodeURIComponent(authUser?.email || '')}&name=${encodeURIComponent(profile?.name || authUser?.user_metadata?.full_name || '')}&class=${encodeURIComponent(profile?.education_level || '')}&school=${encodeURIComponent(profile?.institution_name || '')}`}>
              <button className="bg-brand-orange text-white font-bold px-6 py-3 rounded-xl shadow-lg text-sm">
                Take Assessment
              </button>
            </a>
