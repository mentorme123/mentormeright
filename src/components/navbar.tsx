...
import { ChevronDown, Menu, X, Search, User, LogOut, ClipboardList, ChevronRight, Settings } from "lucide-react";
              ...
              <div className="relative profile-dropdown">
                ...
                >
                  ...
                  <span className="hidden md:inline text-sm font-bold text-slate-700">
                    Hi, {(user.user_metadata?.full_name || user.email || 'User').split(' ')[0]}
                  </span>
                ...
                {profileOpen && (
                  <div className="absolute right-0 mt-2 w-64 bg-white border border-slate-200 rounded-2xl shadow-2xl z-50 overflow-hidden">
                    <div className="p-4 border-b border-slate-100 bg-slate-50">
                      <p className="font-bold text-slate-800 text-sm truncate">{user.user_metadata?.full_name || user.email || 'User'}</p>
                      <p className="text-xs text-slate-500 truncate">{user.email || ''}</p>
                    </div>
                    <div className="p-2">
                      <Link href="/profile" onClick={() => setProfileOpen(false)} className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-50 transition-colors">
                      ...
                      <Link href="/tasks" onClick={() => setProfileOpen(false)} className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-50 transition-colors">
                      ...
                      <button type="button" onClick={() => { setProfileOpen(false); handleLogout(); }} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-red-50 transition-colors border border-red-200">
